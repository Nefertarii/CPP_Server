## 随便写写
2023-5-07  
回头看该程序时，发现除了html页面好点，其他的都不行了，历时一个月将程序全部进行了重置  
版本升级到了2.0!  
更新了全部的文件结构以及函数、变量名称  
更新了日志库的处理逻辑  
更新了相应的测试代码和描述文件  
...
2022-8-05  
发现了之前sendfile问题并进行了修复，原因：使用的是non-blocking套接字并且输出缓冲区已满  
和猜想一致但修改后避免了每次都进行等待提升了效率  
同时修复了许多小问题  

2022-8-03  
更新了程序的启动方式，添加了一个命令选项方便操作  

2022-6-20  
近期修复了大量问题，同时将HTML，JS全部修改完善  
服务器上的传输问题原因不是特别清楚，猜测可能是一时发送的数据过多导致发送缓存区满了导致后续数据被丢弃  
通过测试的结果和猜测很贴近，数据发送不完整但连接状态还是处于Connect的   
目前是通过设置了一下线程sleep将数据发送的流量和宽带保持了一致，就能正常使用了   
该项在```File:CPP_Server/Net/Tcp/tcpconnection.cpp,line:126```中设置  

2022-6-17   
补充更新：在移植到服务器上运行有问题，传输没完成就会被关闭连接  
目前不知道问题是什么原因，同时修复了HTML的一些问题和一些格式  
这几天处理许多小问题，主要在前端页面上花了不少时间    
目前整体到达预期了，项目将会暂缓更新   
之后的更新将暂时只处理发现的问题   

2022-6-16   
更新已经完成了，同步测试和修改了页面和服务器的逻辑  
前端页面修改了部分动画效果，可能还有需要修改的地方，但应该都是小问题   

2022-6-13  
HTML页面对登陆的修改已经能正常使用了，同时修复了js之前遗留的一些问题  
注册页的js和服务器逻辑还需要修改   
