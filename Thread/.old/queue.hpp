...
template <typename T>
typename Wasi::Base::Safe_Queue<T>::Node* Wasi::Base::Safe_Queue<T>::get_tail() {
    std::lock_guard<std::mutex> lk(tail_mtx);
    return tail;
}

template <typename T>
std::unique_lock<std::mutex> Wasi::Base::Safe_Queue<T>::wait_data() {
...
template <typename T>
std::unique_ptr<typename Wasi::Base::Safe_Queue<T>::Node> Wasi::Base::Safe_Queue<T>::pop_head() {
...
template <typename T>
bool Wasi::Base::Safe_Queue<T>::try_pop(T& value) {
...
template <typename T>
std::shared_ptr<T> Wasi::Base::Safe_Queue<T>::try_pop() {
    std::lock_guard<std::mutex> head_lk(head_mtx);
...
template <typename T>
std::shared_ptr<T> Wasi::Base::Safe_Queue<T>::wait_pop() {
...
template <typename T>
void Wasi::Base::Safe_Queue<T>::wait_pop(T& value) {
    std::unique_lock<std::mutex> head_lk(wait_data());
    value                                = std::move(*head->data);
    std::unique_ptr<Node> const old_head = pop_head();
}
...
template <typename T>
void Wasi::Base::Safe_Queue<T>::push(T value) {
...
template <typename T>
bool Wasi::Base::Safe_Queue<T>::empty() {
    std::lock_guard<std::mutex> head_lk(head_mtx);
