...
HttpHeadConfig& HttpHeadConfig::operator=(const HttpHeadConfig& new_config) {
...
std::string StrCode::StrErrorCode(SERVERR err_code) {
    return Serverr_map[-err_code % ERREND];
}

std::string StrCode::StrStateCode(SERVSTATE state_code) {
    return Servstate_map[state_code % STATEND];
}

std::string StrCode::StrTypeCode(REQUESTYPE requset_type) {
    return Request_map[requset_type % TYPEND];
}
