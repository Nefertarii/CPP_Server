#ifndef QUEUE_H_
#define QUEUE_H_

#include <algorithm>
//array based queue
template <typename T>
class Queue
{
private:
    int size_;
    int capacity_;
    int front_;
    int back_;
    T *objects_;
    static const int SPARE_CAPACITY = 16;
    void reserve(int newCapacity);

public:
    explicit Queue(int initSize = 0) : size_(initSize), capacity_(initSize + SPARE_CAPACITY) { objects_ = new T[capacity_]; }
    Queue(const Queue &rhs);
    Queue(Queue &&rhs);
    Queue &operator=(const Queue &rhs);
    Queue &operator=(Queue &&rhs);
    T &front() { return objects_[front_]; }
    int size() const { return size_; }
    bool empty() const { return size() == 0; }
    void push(const T &val);
    void push(T &&val);
    //void emplace();
    void pop();
    ~Queue() { delete[] objects_; }
};

template <typename T>
void Queue<T>::reserve(int newCapacity)
{
    if (newCapacity < size_)
        return;
    T *newArray = new T[newCapacity];
    for (int i = 0; i != capacity_; i++)
    {
        newArray[i] = std::move(objects_[i]);
    }
    capacity_ = newCapacity;
    std::swap(objects_, newArray);
    delete[] newArray;
}

template <typename T>
Queue<T>::Queue(const Queue<T> &rhs)
{
    size_ = rhs.size_;
    capacity_ = rhs.capacity_;
    objects_ = new T[capacity_];
    for (int i = 0; i < size_; i++)
    {
        objects_[i] = rhs.objects_[i];
    }
}

template <typename T>
Queue<T>::Queue(Queue<T> &&rhs)
{
    size_ = rhs.size_;
    rhs.size_ = 0;

    capacity_ = rhs.capacity_;
    rhs.capacity_ = 0;

    objects_ = rhs.objects_;
    rhs.objects_ = nullptr;
}

template <typename T>
Queue<T> &Queue<T>::operator=(const Queue<T> &rhs)
{
    Queue copy = rhs;
    std::swap(*this, copy);
    return *this;
}

template <typename T>
Queue<T> &Queue<T>::operator=(Queue<T> &&rhs)
{
    std::swap(size_, rhs.size_);
    std::swap(capacity_, rhs.capacity_);
    std::swap(objects_, rhs.objects_);
    return *this;
}

template <typename T>
void Queue<T>::push(const T &val)
{
    if (size_ == capacity_)
        reserve(capacity_ * 2 + 1);
    objects_[size_] = val;
    size_ = (size_ + 1) % capacity_;
}

template <typename T>
void Queue<T>::push(T &&val)
{
    if (size_ == capacity_)
        reserve(capacity_ * 2 + 1);
    objects_[size_++] = std::move(val);
}

template <typename T>
void Queue<T>::pop()
{
    front_ = (front_ + 1) % capacity_;
}

#endif