...
namespace Thread {
    ...
    public:
        ...
        void clear();
        uint size();
        void swap(Thread::Safe_List<T>& rhs);
...
template <typename T>
typename std::list<T>::iterator Thread::Safe_List<T>::find(const T& value) {
...
template <typename T>
void Thread::Safe_List<T>::insert(const T& posi_value, const T& insert_value) {
    std::lock_guard<std::mutex> lk(mtx);
...
template <typename T>
void Thread::Safe_List<T>::earse(const T& posi_value) {
    std::lock_guard<std::mutex> lk(mtx);
    auto ret = find(posi_value);
    if (ret != data.end()) {
        data.erase(ret);
...
template <typename T>
void Thread::Safe_List<T>::swap(Thread::Safe_List<T>& rhs) {
...
template <typename T>
typename std::list<T>::iterator Thread::Safe_List<T>::begin() { return data.begin(); }

template <typename T>
typename std::list<T>::iterator Thread::Safe_List<T>::end() { return data.end(); }

template <typename T>
void Thread::Safe_List<T>::lock() { mtx.lock(); }

template <typename T>
void Thread::Safe_List<T>::unlock() { mtx.unlock(); }
